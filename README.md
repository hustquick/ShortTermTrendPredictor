...
- 高置信基础阈值：`up_probability >= 0.80` 或 `up_probability <= 0.30`。
- 连续确认过滤：做多要求最近 5 分钟内至少 3 次 `up_probability >= 0.90`，且 `ret_5 < 0`、`close_position > 0.70`，两次有效信号至少间隔 10 分钟。
- 当前实盘信号配置：只启用做多连续确认信号；做空侧在密集短测中不稳定，暂时禁用。
